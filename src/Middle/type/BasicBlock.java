...
public class BasicBlock implements Comparable<BasicBlock> {
    ...
    private HashSet<Symbol> localSymbolUsage = null;
    ...
    // 记录全局变量（LOCAL变量）
    // DEF
    public void addToDefSet(Operand operand) {
    ...
    // USE
    public void addToUseSet(Operand operand) {
        if (!(operand instanceof Symbol && ((Symbol) operand).getScope() == Symbol.Scope.LOCAL)) {
            return;
        }
        Symbol localSymbol = (Symbol) operand;
    ...
    public void addContent(BlockNode blockNode) {
        ...
        if (blockNode instanceof Branch) {
            ...
            for (Operand param : ((FuncCall) blockNode).getrParams()) {
                addToUseSet(param);
            }
    ...
    // TODO: 记录全部变量被 ***使用*** 的次数（包括临时变量和局部变量）
    ...
    public HashSet<Symbol> getLocalSymbolUsage() {
        if (localSymbolUsage != null) {
            return localSymbolUsage;
        }
        localSymbolUsage = new HashSet<>();
        for (Operand operand : operandUsage) {
            if (operand instanceof Symbol && ((Symbol) operand).getScope() == Symbol.Scope.LOCAL) {
                localSymbolUsage.add((Symbol) operand);
            }
        }
        return localSymbolUsage;
    }
